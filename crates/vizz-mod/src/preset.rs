...
pub const BUILTINS: &[Builtin] = &[
    ...
    // --- Vector looks: the print-side counterparts to the six above.
    // Each turns the particle field off and states every layer it uses in
    // full, plus kind 0 for the rest — a vector look must arrive whole
    // from any prior state, exactly as Tunnel does for the field.
    Builtin {
        name: "Interference",
        about: "Two ring fields multiplied into moir\u{e9}. The print look's opener.",
        values: &[
            ("/l1/kind", 1.0),
            ("/l1/freq", 14.0),
            ("/l1/phase", 0.0),
    ...
    Builtin {
        name: "Poster",
        about: "A star cut out of a checkerboard by exclusion. Hard geometry, screen-print flat.",
        values: &[
            ("/l1/kind", 3.0),
            ("/l1/freq", 6.0),
            ("/l1/phase", 0.0),
    ...
    Builtin {
        name: "Signal",
        about: "Rays through a dot grid on dark, screened, punched by an inverted hexagon.",
        values: &[
            ("/l1/kind", 7.0),
            ("/l1/freq", 9.0),
            ("/l1/phase", 0.0),
